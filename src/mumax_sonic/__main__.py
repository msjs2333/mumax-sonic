...
def main():
    ...
    parser.add_argument('--band-low-ghz', type=float, default=8)
    parser.add_argument('--band-high-ghz', type=float, default=12)
    ...
    if args.recipe == 'band' or (args.field_demo and args.field_demo.startswith('band_')) or not (args.doctor or args.audio_smoke is not None):
        ...
        try:
            band_config = BandConfig(args.band_low_ghz*1e9, args.band_high_ghz*1e9, args.band_window, tuple(args.band_axis))
        except (ValueError, TypeError) as exc:
            parser.error(str(exc))
    if not math.isfinite(args.band_reference) or args.band_reference <= 0:
        parser.error('--band-reference must be finite and positive')
    ...
    if args.audio_smoke and args.field_demo and not args.field_demo.startswith(('activity_', 'band_')):
            ...
            if args.audio_smoke is not None:
                while (elapsed := time.monotonic()-start) < args.audio_smoke:
                    ...
                    if args.field_demo and args.field_demo.startswith('band_'):
                        ...
                        sample = view.sample
                        report['band'] = view.diagnostic
                        reference = args.band_reference
